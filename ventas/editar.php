...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errores)) {
        ...
        if (mysqli_query($conexion, $query)) {
            ...
            $errores[] = "Error al actualizar la venta: " . mysqli_error($conexion);
        }
    }
}
    ...
    <form method="post" action="">
        <div class="form-group">
            ...
            <select id="id_cliente" name="id_cliente" class="form-control" required>
                ...
                <?php mysqli_data_seek($resultado_clientes, 0); ?>
                <?php while ($cliente = mysqli_fetch_assoc($resultado_clientes)): ?>
                    <option value="<?php echo $cliente['id_cliente']; ?>"
                        <?php echo ((isset($_POST['id_cliente']) && $_POST['id_cliente'] == $cliente['id_cliente']) ||
                                  (!isset($_POST['id_cliente']) && $venta['id_cliente'] == $cliente['id_cliente'])) ? 'selected' : ''; ?>>
            ...
            <select id="id_producto" name="id_producto" class="form-control" required>
                ...
                <?php while ($producto = mysqli_fetch_assoc($resultado_productos)): ?>
                    <option value="<?php echo $producto['id_producto']; ?>"
                        <?php echo ((isset($_POST['id_producto']) && $_POST['id_producto'] == $producto['id_producto']) ||
                                  (!isset($_POST['id_producto']) && $venta['id_producto'] == $producto['id_producto'])) ? 'selected' : ''; ?>>
        ...
        <div class="form-group">
            ...
            <input type="date" id="fecha_venta" name="fecha_venta" class="form-control"
                   value="<?php echo isset($_POST['fecha_venta']) ? $_POST['fecha_venta'] : $venta['fecha_venta']; ?>" required>
        </div>

        <div class="form-group">
            <label for="cantidad">Cantidad:</label>
            <input type="number" id="cantidad" name="cantidad" class="form-control"
                   value="<?php echo isset($_POST['cantidad']) ? $_POST['cantidad'] : $venta['cantidad']; ?>" min="1" required>
        </div>

        <div class="form-group">
            <label for="precio_unitario">Precio Unitario:</label>
            <input type="number" step="0.01" id="precio_unitario" name="precio_unitario" class="form-control"
                   value="<?php echo isset($_POST['precio_unitario']) ? $_POST['precio_unitario'] : $venta['precio_unitario']; ?>" required>
        </div>

        <div class="form-group">
            <input type="submit" value="Actualizar Venta" class="btn btn-submit">
...
mysqli_free_result($resultado_venta);
